...
public class ProductoController implements Serializable {
        ...
        if (pagination == null) {
            pagination = new PaginationHelper(10) {
                ...
                public int getItemsCount() {
                    return getFacade().countProducto();
                }

                @Override
                public DataModel createPageDataModel() {
                    return new ListDataModel(getFacade().findProductoRange(new int[]{getPageFirstItem(), getPageFirstItem() + getPageSize()}));
                }
            };
        }
        return pagination;
    ...
    public String create() {
        try {
            getFacade().createProducto(current);
    ...
    public String update() {
        try {
            getFacade().editProducto(current);
    ...
    private void performDestroy() {
        try {
            getFacade().removeProducto(current);
    ...
    private void updateCurrentItem() {
        int count = getFacade().countProducto();
        ...
        if (selectedItemIndex >= 0) {
            current = getFacade().findProductoRange(new int[]{selectedItemIndex, selectedItemIndex + 1}).get(0);
        }
    }
    ...
    public SelectItem[] getItemsAvailableSelectMany() {
        return JsfUtil.getSelectItems(ejbFacade.findAllProducto(), false);
    }

    public SelectItem[] getItemsAvailableSelectOne() {
        return JsfUtil.getSelectItems(ejbFacade.findAllProducto(), true);
    }

    public Producto getProducto(java.lang.Long id) {
        return ejbFacade.findProducto(id);
    }
