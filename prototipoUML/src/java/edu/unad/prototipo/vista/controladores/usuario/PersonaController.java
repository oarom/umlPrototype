...
public class PersonaController implements Serializable {
        ...
        if (pagination == null) {
            pagination = new PaginationHelper(10) {
                ...
                public int getItemsCount() {
                    return getFacade().countPersona();
                }

                @Override
                public DataModel createPageDataModel() {
                    return new ListDataModel(getFacade().findPersonaRange(new int[]{getPageFirstItem(), getPageFirstItem() + getPageSize()}));
                }
            };
        }
        return pagination;
    ...
    public String create() {
        try {
            getFacade().createPersona(current);
    ...
    private void performDestroy() {
        try {
            getFacade().removePersona(current);
    ...
    private void updateCurrentItem() {
        int count = getFacade().countPersona();
        ...
        if (selectedItemIndex >= 0) {
            current = getFacade().findPersonaRange(new int[]{selectedItemIndex, selectedItemIndex + 1}).get(0);
        }
    }
    ...
    public SelectItem[] getItemsAvailableSelectMany() {
        return JsfUtil.getSelectItems(ejbFacade.findAllPersona(), false);
    }

    public SelectItem[] getItemsAvailableSelectOne() {
        return JsfUtil.getSelectItems(ejbFacade.findAllPersona(), true);
    }

    public Persona getPersona(java.lang.Long id) {
        return ejbFacade.findPersona(id);
    }
