package edu.unad.prototipo.vista.controladores.usuario;
...
import edu.unad.prototipo.vista.facades.UsuarioFacade;
...
public class DireccionController implements Serializable {
        ...
        if (pagination == null) {
            pagination = new PaginationHelper(10) {
                ...
                public int getItemsCount() {
                    return getFacade().countDireccion();
                }

                @Override
                public DataModel createPageDataModel() {
                    return new ListDataModel(getFacade().findDireccionRange(new int[]{getPageFirstItem(), getPageFirstItem() + getPageSize()}));
                }
            };
        }
        return pagination;
    ...
    public String create() {
        try {
            getFacade().createDireccion(current);
    ...
    public String update() {
        try {
            getFacade().editDireccion(current);
    ...
    private void performDestroy() {
        try {
            getFacade().removeDireccion(current);
    ...
    private void updateCurrentItem() {
        int count = getFacade().countDireccion();
        ...
        if (selectedItemIndex >= 0) {
            current = getFacade().findDireccionRange(new int[]{selectedItemIndex, selectedItemIndex + 1}).get(0);
        }
    }
    ...
    public SelectItem[] getItemsAvailableSelectMany() {
        return JsfUtil.getSelectItems(ejbFacade.findAllDireccion(), false);
    }

    public SelectItem[] getItemsAvailableSelectOne() {
        return JsfUtil.getSelectItems(ejbFacade.findAllDireccion(), true);
    }

    public Direccion getDireccion(java.lang.Long id) {
        return ejbFacade.findDireccion(id);
    }
