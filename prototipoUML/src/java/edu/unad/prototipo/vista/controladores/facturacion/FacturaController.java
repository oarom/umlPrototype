...
public class FacturaController implements Serializable {
        ...
        if (pagination == null) {
            pagination = new PaginationHelper(10) {
                ...
                public int getItemsCount() {
                    return getFacade().countFactura();
                }

                @Override
                public DataModel createPageDataModel() {
                    return new ListDataModel(getFacade().findFacturaRange(new int[]{getPageFirstItem(), getPageFirstItem() + getPageSize()}));
                }
            };
        }
        return pagination;
    ...
    public String create() {
        try {
            getFacade().createFactura(current);
    ...
    private void performDestroy() {
        try {
            getFacade().removeFactura(current);
    ...
    private void updateCurrentItem() {
        int count = getFacade().countFactura();
        ...
        if (selectedItemIndex >= 0) {
            current = getFacade().findFacturaRange(new int[]{selectedItemIndex, selectedItemIndex + 1}).get(0);
        }
    }
    ...
    public SelectItem[] getItemsAvailableSelectMany() {
        return JsfUtil.getSelectItems(ejbFacade.findAllFactura(), false);
    }

    public SelectItem[] getItemsAvailableSelectOne() {
        return JsfUtil.getSelectItems(ejbFacade.findAllFactura(), true);
    }

    public Factura getFactura(java.lang.Long id) {
        return ejbFacade.findFactura(id);
    }
